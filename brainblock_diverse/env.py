...
from brainblock_diverse.pieces import (
    ALL_PIECES,
    COLS,
    INITIAL_INVENTORY,
    N_ACTIONS,
...
class BrainBlockDiverseEnv(gym.Env):
    def __init__(self, solution_memory=None):
        super().__init__()
    ...
    def get_info(self):
        return {
            ...
            "unique_solutions": 0 if self.solution_memory is None else len(self.solution_memory),
        }
    ...
    def step(self, action):
        ...
        if len(self.queue) == 0:
            ...
            max_similarity = 0.0
            signature = board_signature(self.board)

            if self.solution_memory is not None:
                diversity_penalty, is_new_solution, max_similarity = self.solution_memory.penalty_for(signature)
                self.solution_memory.register(signature, self.board)
            ...
            info["is_new_solution"] = is_new_solution
            info["max_solution_similarity"] = max_similarity
